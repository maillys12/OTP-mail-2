...
const BACKEND_VERSION = '2026.07.30-country-v4';
...
function dispatch_(action, p) {
  switch (action) {
    ...
    case 'getCountryCatalog':
      return getCountryCatalog_(p);
...
function healthCheck_() {
  ...
  return {
    success: true,
    version: BACKEND_VERSION,
    usersHeaders: headerRow_(sheet_('Users'))
  };
}
...
function rows_(name) {
  ...
  return values.slice(1).map(function(row, i) {
    ...
    alias_(obj, 'passwordSalt', [
      'password_salt'
    ]);

    alias_(obj, 'sessionHash', [
      'session_hash',
      'sessionTokenHash',
      'session_token_hash'
    ]);

    alias_(obj, 'sessionExpiresAt', [
      'session_expires_at',
      'sessionExpiry',
      'session_expiry'
    ]);
...
function loginUser_(p) {
  ...
  if (
    !isPasswordHash_(u.passwordHash) ||
    !u.passwordSalt
  ) {
    return {
      success: false,
...
function createSessionResponse_(u) {
  ...
  if (
    !saved.sessionHash ||
    !constantTimeEqual_(
      String(saved.sessionHash),
      sha256_(token)
    )
  ) {
    throw new Error(
      'บันทึก Session ไม่สำเร็จ กรุณารัน setupSheets() แล้ว Deploy Apps Script เวอร์ชันใหม่'
    );
  }
